...
#
# The scripts are intentionally no-argument. Edit the variables below, or
# override them from the shell before running a step.
...
NAV_ANNOTATIONS="${NAV_ANNOTATIONS:-/home/user/Workspace/alpamayo1.5/notebooks/nav_demo_samples.json}"
...
run_in_tmux_by_default() {
  ...
  if [[ -n "${TMUX:-}" || "${A15_SFT_DISABLE_TMUX:-no}" == "yes" ]]; then
    return
  fi

  if ! command -v tmux >/dev/null 2>&1; then
    log "tmux is required for the default execution path but was not found on PATH"
    log "install tmux, or run with A15_SFT_DISABLE_TMUX=yes to execute in the current shell"
    exit 127
  fi

  log "starting inside tmux session: ${session_name}"
  log "attach later with: tmux attach -t ${session_name}"
  exec tmux new-session -A -s "${session_name}" "${script_path}"
}
...
activate_venv() {
  if [[ ! -x "${VENV_DIR}/bin/python" ]]; then
    ...
    log "run scripts/a15_sft_readme_00_setup_env.sh first"
    exit 2
  fi
